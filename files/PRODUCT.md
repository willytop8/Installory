...
Installory is a **trustworthy guide to what's actually on your Mac and what's safe to remove.**
...
3. **Cleanup scripts.** When you decide to remove something, Installory generates a shell script with the exact commands — annotated, ordered, and safe to read — that you run yourself in Terminal. Installory never touches your packages directly. This is the safer design: you see every command before it executes, in the tool you already trust for irreversible work.

## What we are not

Installory is **not**:

- A package manager itself — we don't install, upgrade, **or uninstall**. Cleanup is generated as a script you run yourself; Installory never modifies your packages directly.
...
What makes Installory different from the dozen existing package-management tools:
...
2. **Provenance via Claude Code logs.** For the subset of users who code with Claude Code, we can reconstruct exactly when and why a package was installed by reading `~/.claude/projects/<...>.jsonl`. This is unique to Installory and only became feasible in the last year.
...
- **Reversible by default.** We never run destructive commands ourselves — you do, in your Terminal, with full visibility. Before any cleanup script is generated, Installory captures a snapshot you can export as a reinstall script later.
