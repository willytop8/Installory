...
The bundled `descriptions.db` (shipped in the app bundle, read-only) is a single table:
...
`DescriptionStore` reads from the bundled DB only. There is no writable descriptions table — the corpus is the single source of truth, and it's regenerated by the maintainer between releases.
...
**Why a separate `bundled_descriptions` DB instead of seeding the user's DB?**
