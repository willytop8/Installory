...
public struct ScriptGenerator: Sendable {
    ...
    private func appendHeader(to out: inout [String], snapshot: SnapshotContext?) {
        ...
        out.append("#!/bin/bash")
        out.append("# Generated by Backshelf on \(fmt.string(from: Date()))")
        out.append("# Review every line before running. You can run this script manually")
        out.append("# in Terminal — Backshelf does not execute it for you.")
        ...
        if let snapshot {
            ...
            out.append("# snapshot in Backshelf and use 'Export as reinstall script'.")
        }
    ...
    private func appendDenylistSection(packages: [Package], to out: inout [String]) {
        ...
        out.append("# software. Backshelf has commented them out. Uncomment only if you are")
    ...
    private func echoLine(for cmd: String) -> String {
        let escaped = cmd
    ...
    private func shellDoubleQuoteEscape(_ s: String) -> String {
        s
            .replacingOccurrences(of: "\\", with: "\\\\")
            .replacingOccurrences(of: "\"", with: "\\\"")
            .replacingOccurrences(of: "`", with: "\\`")
